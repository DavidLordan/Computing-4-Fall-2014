...
void writeHTML(Element* currentElement, ofstream &os) {
    ...
    if (currentElement != emptyRoot) {
        ...
        if (currentElement->hasAttributes()) {
            ...
            for (int i = 0; i < currentElement->vecAttribute.size(); i++) {
                os << "</div>";
                os << "<div>";
                ...
                os << "Attribute name: " << currentElement->vecAttribute[i]->getAttName();
                os << ", Attribute value: " << currentElement->vecAttribute[i]->getAttValue();
                os << "</div>";
            }
...
int main(int argc, char** argv) {
    ...
    // Asks the user if they would like the tree building process to be displayed. 
    askFormat();

    // If the XMl file was opened successfully, the element tree will be displayed and then deleted.
    if (openfile("Assignment4_musicFile.xml") == EXIT_SUCCESS) {
        displayTree(currentElement);

        /* ofstream object which is created by the 'openHTML' function. This is done to avoid
         * creating a global ofstream object. */
        ofstream htmlStream;
        htmlStream.open("HTML_OUTPUT.html");
